    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          text: { type: 'string', description: 'The text content of what is being spoken (for transcript logging). If omitted, speech is not logged to the session transcript.' },
    ...
    switch (req.params.name) {
      ...
      case 'speak': {
        ...
        try {
          ...
          // Log bot speech to the session transcript (if text was provided)
          if (spokenText) {
            const teardownFn = (connection as any)._voiceReceiveTeardown
            if (teardownFn?.logBotSpeech) {
              teardownFn.logBotSpeech(spokenText)
            }
          }
