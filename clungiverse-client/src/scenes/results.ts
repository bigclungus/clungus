...
function renderPlayerBreakdown(
  ...
  for (const pr of results.players) {
    ...
    ctx.fillText(pr.totalHealing > 0 ? `+${String(pr.totalHealing)}` : '-', colX + 350, sy);
    ctx.fillStyle = '#cccccc';
    ctx.fillText(pr.diedOnFloor !== null ? `F${String(pr.diedOnFloor)}` : '-', colX + 410, sy);
...
export function createResultsScene(
  ...
  return {
    ...
    render(state: DungeonClientState, ctx: CanvasRenderingContext2D): void {
      ...
      ctx.fillText(`Floor Reached: ${String(results.floorReached)} / ${String(results.totalFloors)}`, w / 2, sy);
      sy += 24;
      ...
      ctx.fillText(`Total Kills: ${String(results.kills)}`, w / 2, sy);
      sy += 24;

      ctx.fillText(`Damage Dealt: ${String(results.damageDealt)}`, w / 2, sy);
      sy += 24;

      ctx.fillText(`Damage Taken: ${String(results.damageTaken)}`, w / 2, sy);
      sy += 40;

      sy = renderPlayerBreakdown(ctx, state, w, sy);
