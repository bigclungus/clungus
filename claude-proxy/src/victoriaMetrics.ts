...
function safeUsageJson(usage: Record<string, unknown>): string {
  try {
    return JSON.stringify(usage);
  } catch (e) {
    return `[usage not JSON-serializable: ${e instanceof Error ? e.message : String(e)}]`;
  }
}

/**
 * Log merged API usage as JSON and push each numeric field to VictoriaMetrics (no derived totals).
 * Never throws (streaming tap must not fail the client response).
 */
export function pushUsageMetrics(
  ...
  try {
    emitLog(`[claude-proxy] recorded usage ${safeUsageJson(usage)}`);
    ...
    void fetch(url, {
      ...
      emitLog(
        `[claude-proxy] VictoriaMetrics push failed: ${e instanceof Error ? e.message : String(e)}`,
      );
    });
  } catch (e) {
    emitLog(
      `[claude-proxy] usage metrics skipped: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}
