...
Bun.serve({
  ...
  async fetch(req) {
    ...
    // SSE / JSON: tap to normalize `usage` (clients dereference `usage.input_tokens`; upstream
    // often omits `usage` or sends null on some events; non-stream bodies used to bypass the tap).
    if (log.response || vmOn || isSse || isJson) {
